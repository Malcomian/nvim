return {
  ...
  config = function()
    ...
    set({ "n", "x" }, "<leader>N", function()
      mc.matchAddCursor(-1)
    end, { desc = "Multicursor: Add cursor at previous match" })
    set({ "n", "x" }, "<leader>S", function()
      mc.matchSkipCursor(-1)
    end, { desc = "Multicursor: Skip to previous match" })

    -- Add and remove cursors with control + left click.
    set("n", "<c-leftmouse>", mc.handleMouse, { desc = "Multicursor: Handle mouse (Ctrl+click)" })
    set("n", "<c-leftdrag>", mc.handleMouseDrag, { desc = "Multicursor: Handle mouse drag" })
    set("n", "<c-leftrelease>", mc.handleMouseRelease, { desc = "Multicursor: Handle mouse release" })
    ...
    mc.addKeymapLayer(function(layerSet)
      ...
      end, { desc = "Multicursor: Toggle enable / Clear all cursors" })
    end)
